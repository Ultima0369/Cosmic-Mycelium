...
class SuperBrain:
    ...
    def _apply_pathway_influence(
        self, source_name: str, influence: float = 0.1
    ) -> None:
            ...
            if pathway.source == source_name:
                ...
                if target:
                    ...
                    pathway.weight = min(
                        1.0,
                        pathway.weight
                        + pathway.plasticity * source_activation * target.activation,
                    )
                    # Propagate activation scaled by pathway weight
                    target.activation = min(
                        1.0,
                        target.activation
                        + influence * source_activation * pathway.weight,
                    )
    ...
    def plan(
        self, goal: dict | None = None, options: list[dict] | None = None
    ) -> dict | None:
        """
    ...
    def broadcast_global_workspace(self, content: dict, priority: float = 0.6) -> bool:
        ...
        if self.competition_enabled:
            ...
            source = (
                max(self.regions.items(), key=lambda item: item[1].activation)[0]
                if self.regions
                else None
            )
        ...
        if "meta" in self.regions:
            self.regions["meta"].working_memory.append(
                content
            )  # Raw content for legacy query
    ...
    def get_region_health(self) -> dict[str, dict[str, float]]:
        ...
        for name, region in self.regions.items():
            activations = (
                region.activation_history[-20:]
                if region.activation_history
                else [region.activation]
            )
            health[name] = {
                "activation_mean": float(np.mean(activations)),
                "activation_variance": float(np.var(activations)),
                "stagnation": float(stagnation[name]),
                "memory_utilization": len(region.working_memory)
                / region.working_memory.maxlen,
            }
        return health
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "global_workspace_source": (
                self.global_workspace.get("source") if self.global_workspace else None
            ),

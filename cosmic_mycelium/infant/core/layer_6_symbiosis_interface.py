...
class Interaction:
	...
	successful: Optional[bool] = None  # Outcome assessment
...
class SymbiosisInterface:
	...
	def _update_trust_decay(self) -> None:
		...
		for partner in self.partners.values():
			...
			if hours_idle > self.trust_decay_hours:
				...
				decay = math.exp(-partner.trust_decay_rate * (hours_idle - self.trust_decay_hours))
	...
	def propose_value(self, proposal_type: str, content: Dict, recipient: str, expiry: Optional[float] = None) -> Dict:
		"""
		...
		self.history.append(f"Proposed {proposal_type} to {recipient} (id={proposal_id})")
		return {
			"proposal_id": proposal_id,
			"status": "pending",
			"expiry": expiry_time,
	...
	def accept_proposal(self, proposal_id: str, increase: float = 0.1) -> Dict:
		...
		return {"status": "accepted", "partner": partner_id, "trust": partner.trust, "committed": committed}

	def reject_proposal(self, proposal_id: str, decrease: float = 0.1, reason: Optional[str] = None) -> Dict:
		"""
		...
		for msg in self.inbox:
			...
			if msg_type == "QUERY":
				...
				if partner in self.partners:
					self.partners[partner].quality_score = min(1.0, self.partners[partner].quality_score + 0.05)
				...
				if prop_id in self.active_negotiations:
					...
					if partner in self.partners:
						self.partners[partner].quality_score = min(1.0, self.partners[partner].quality_score + 0.1)
				...
				if prop_id in self.active_negotiations:
					...
					neg.status = "rejected"
					if partner in self.partners:
						self.partners[partner].quality_score = max(0.0, self.partners[partner].quality_score - 0.1)
	...
	def explain_state(self) -> str:
		...
		for pid, p in self.partners.items():
			partner_info.append(f"{pid} (trust={p.trust:.2f}, {p.mode.value}, {p.status.value})")
		partners_str = ", ".join(partner_info) if partner_info else "none"
		active_negs = len([n for n in self.active_negotiations.values() if n.status == "pending"])
		return (
	...
	def explain_decision(self, action: Dict) -> str:
		...
		partner_ctx = f"with {len(active)} trusted partners" if active else "in solo mode"
		return (
	...
	def get_status(self) -> Dict:
		...
		return {
			...
			"pending_negotiations": len([n for n in self.active_negotiations.values() if n.status == "pending"]),
			"history_len": len(self.history),
			"avg_interaction_quality": (self._interaction_quality_sum / self._interaction_count) if self._interaction_count > 0 else 0.0,
		}

...
class TestRunInfantArgumentParsing:
    ...
    def test_parse_args_all_custom(self):
        ...
        with patch.object(sys, 'argv', [
            ...
            '--log-level', 'DEBUG',
            '--energy-max', '200',
            '--physics-drift-max', '0.005',
            '--physics-adapt-thresh', '0.002',
            '--metrics-port', '9090',
            '--health-port', '9091',
        ]):
            args = parse_args()
...
class TestRunInfantImports:
    ...
    def test_imports_success(self):
        try:
            from cosmic_mycelium.scripts.run_infant import main, parse_args, InfantConfig, InfantRunner
...
class TestInfantRunnerInitialization:
    ...
    def test_runner_initializes_with_config(self):
        from cosmic_mycelium.scripts.run_infant import InfantRunner, InfantConfig
...
class TestInfantRunnerStart:
    ...
    async def test_start_creates_infant_and_servers(self):
        from cosmic_mycelium.scripts.run_infant import InfantRunner, InfantConfig, MetricsServer, HealthChecker
        cfg = InfantConfig(infant_id="test-start")
        runner = InfantRunner(cfg)

        with patch.object(MetricsServer, 'start', new=AsyncMock()), \
             patch.object(HealthChecker, 'start', new=AsyncMock()), \
             patch.object(runner, '_wait_for_shutdown', new=AsyncMock()) as mock_wait, \
             patch.object(runner, 'shutdown', new=AsyncMock()) as mock_shutdown:
            # Mock the infant creation and run to avoid actual execution
            with patch.object(runner, '_run_infant', new=AsyncMock()):
                await runner.start()
    ...
    async def test_start_sets_running_flag(self):
        ...
        with patch.object(MetricsServer, 'start', new=AsyncMock()), \
             patch.object(HealthChecker, 'start', new=AsyncMock()), \
             patch.object(runner, '_wait_for_shutdown', new=AsyncMock(return_value=None)), \
...
class TestInfantRunnerMain:
    ...
    def test_main_creates_config_from_args(self):
        from cosmic_mycelium.scripts.run_infant import parse_args, InfantConfig, main
        with patch('cosmic_mycelium.scripts.run_infant.InfantRunner') as MockRunner, \
